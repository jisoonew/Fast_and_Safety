...
<nav class="sb-topnav navbar navbar-expand navbar-dark bg-dark">
        ...
        <h1 class="mt-4" id="content_title">정기 배송</h1>
		...
		<table class="table table-bordered" style="font-size:15px;">
			...
			<c:forEach items="${routine}" var="routine" varStatus="status">
 			...
 			<td><c:out value="${status.count}"/></td><!-- 테이블 순서 -->
 				<td><c:out value="${routine.rd_kind_release}"/></td><!-- 물품명 -->
  				<td><c:out value="${routine.rd_volume}"/></td><!-- 물량 -->
  				<td><c:out value="${routine.container_type}"/></td><!-- 컨테이너 타입 -->
 				 <td><c:out value="${routine.container_num}"/></td><!-- 컨테이너 번호 -->
 				 <td><c:out value="${routine.rd_name}"/></td><!-- 수령인 -->
 				 <td><c:out value="(${routine.rd_postcode}) ${routine.rd_address} ${routine.rd_detail_address}"/></td><!-- 주소 -->
 				 <td><c:out value="처리"/></td><!-- 배송 상태 -->
 				 <td><c:out value="${routine.rd_delivery_fee}"/></td><!-- 배송비 -->
			</tr>
			</c:forEach>
			</tbody>
		</table>
